...
class Tarefa:
    """
        Classe para repersentar tarefa, com metodos para salvar, obter,
    ...
    def obter_tarefas(cls) -> list[Self]:
        with Database()as db:
            ...
            tarefas: list[Self] = [cls(titulo, data, id) for titulo, data,
            id in resultados]
            return tarefas
    ...
    def concluir_tarefa(self):
        with Database() as db:
            query: str = """UPDATE tarefas SET data_conclusao = ? WHERE id = ?;"""
            params: tuple = ("Concluida", self.id_tarefa)
            resultado: Cursor = db.executar(query, params)
            return resultado